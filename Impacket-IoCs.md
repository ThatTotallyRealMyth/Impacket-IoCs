...
- [Kerberos and ticketing](#cat-kerberos-and-ticketing)
  ...
  - [IoC 09 - `ticketer.py` forged silver tickets do not have pac types 16 and 19 set](#ioc-09)
  - [IoC 10 - `ticketer.py` PAC has fixed `LogonCount = 500`, empty `LogonServer`, and default admin-heavy group RIDs](#ioc-10)
  - [IoC 11 - `ticketer.py` writes KDC reply nonce `123456789` into forged EncRepPart](#ioc-11)
...
- [NTLM and SPNEGO](#cat-ntlm-and-spnego)
  ...
  - [IoC 25.1 - Impacket Sets The `NTLMSSP_NEGOTIATE_TARGET_INFO` flag in the NTLM type 1 message](#ioc-25.1)
  - [IoC 25.2 - Impackets NTLM is Missing `NTLMSSP_NEGOTIATE_OEM` flag in Type 1 messages](#ioc-25.2)
...
### IoC 09 - `ticketer.py` forged silver tickets do not have pac types 16 and 19 set
**Surface:** Kerberos ticket inspect ion, service-side PAC/ticket validation logs where decrypted, endpoint ticket cache forensics

When ticketer.py forges a silver ticket, it constructs the PAC and signs it using the sign_pac() function in impacket/krb5/pac.py. This function only computes two signatures;

- Server Checksum (type 6): a keyed hash of the entire PAC with signature fields zeroed, using the service account key
- KDC Checksum (type 7): a keyed hash of the Server Checksum's signature value

As a result, negating Ticket Checksum (type 16) and  Extended KDC Checksum (type 19). We can observe this when using a silver ticket generated by ticketer with `-extra-pac` argument:

![alt text](images/Screenshot2026-05-10at10.18.26PM.png)

**Expected/Proper Baseline**:

To address Bronze Bit (CVE-2020-17049), Microsoft added the Ticket Checksum (type 16), defined in MS-PAC. Its a keyed hash of the service ticket (with the PAC replaced by a single zero byte), signed with the krbtgt key. The issue at its core was that certain parts of the service ticket were not encrypted/protected like other sections of the ticket. This meant that there were some sections of the ticket that could be modified without the KDC or service finding out. The Ticket Checksum (type 16) was thus introduced to counter this gap because it signs the EncTicketPart (minus the PAC), using the krbtgt key rather than the service key.It made it such that even if you have the service key and can re-encrypt the ticket, you cannot recompute the Ticket Checksum after modifying the flags because you don't have the krbtgt key.

For the second flag, [MS-PAC Section 2.8.3](https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-pac/9cf6f6ad-6b76-44b3-aefa-901aa1ff5a08) defines the Extended KDC Checksum (type 19) as a keyed hash of the entire PAC (with all signature fields zeroed), signed with the krbtgt key. Unlike the KDC Checksum (type 7) which only signs the Server Checksum's signature, type 19 covers all PAC contents directly.

We can see both flags being set by a Windows client connecting to LDAP on the DC:

![](images/Screenshot2026-05-10at9.58.50PM.png)
...
### IoC 10 - `ticketer.py` PAC has fixed `LogonCount = 500`, empty `LogonServer`, and default admin-heavy group RIDs
...
### IoC 11 - `ticketer.py` writes KDC reply nonce `123456789` into forged EncRepPart
...
### IoC 12 - `raiseChild.py` forged parent access injects Enterprise Admin SID `-519` as an ExtraSid
...
### IoC 13 - Kerberos SPNEGO advertises the legacy Microsoft Kerberos OID but wraps the AP-REQ with the standard Kerberos OID
...
### IoC 14 - LDAP Kerberos bind sends raw `AP-REQ` as the SPNEGO mechToken
...
Windows clients, pursuant to the relevant MS-DCOM/MS-WMI specifications spoken about in IoC 43 use a larger activation blob:
